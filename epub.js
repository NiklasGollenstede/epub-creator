...
require('chrome').Cu.importGlobalProperties([ 'Blob', ]); /* globals Blob */
...
EPub.prototype = {
	...
	toBlob() {
		let zip = this.zip();
		try {
			zip = zip.generate({ mimeType, type: 'blob', });
		} catch (error) {
			zip = zip.generate({ mimeType, type: 'uint8array', });
			zip = new Blob([ zip, ], { type: mimeType, });
		}
		return zip;
	},
	/**
	...
	toDataURL() {
		let zip = this.zip();
		zip = zip.generate({ mimeType, type: 'base64', });
		return 'data:application/epub+zip;base64,'+ zip;
	},
};
